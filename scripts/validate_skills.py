#!/usr/bin/env python3
"""Validate Ripple Skill metadata, links, layout, and execution contracts."""
...
SKILLS_DIR = ROOT / "skills" / "ripple"
ALLOWED_KEYS = {"name", "description", "layer"}
...
PUBLISH_SCRIPT_CONTRACTS = {
    "skills/ripple/skill-bilibili-upload/scripts/bili_upload.py": (
        "content_guard.guard_or_die", 'add_argument("--exec"',
    ),
    "skills/ripple/skill-wechat-publisher/scripts/publish.py": (
        "content_guard.guard_or_die", '"--exec"',
    ),
    "skills/ripple/skill-wechat-publisher/scripts/multi_publish.py": (
        "content_guard.guard_or_die", '"--exec"',
    ),
...
def validate(path: Path) -> list[str]:
    ...
    # Code-span references are common in Ripple Skills; validate those too.
...
def validate_execution_contracts() -> list[str]:
    ...
    analyzer_docs = [
        ROOT / "skills/ripple/skill-xhs-analyzer/SKILL.md",
        ROOT / "skills/ripple/skill-xhs-analyzer/references/modules.md",
    ]
...
def validate_output_layout_references() -> list[str]:
    ...
    errors: list[str] = []
    roots = (ROOT / "skills/ripple", ROOT / "skills/shared/scripts")
    for base in roots:
