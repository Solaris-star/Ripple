"""Ripple 画像（Profile）共享助手 —— CLI / Web / skill 三入口的单一真相源。

历史上三处各写一份画像逻辑，且 CLI 还残留全局 USER.md（并发竞态）。
统一到这里后：画像一律**作为消息内联**注入（`我当前使用的画像是「X」。`），
由 Ripple Agent 按当前会话绑定读取 `profiles/<X>/`，每个请求自包含，无全局文件污染。
...
def persona_prefix(name: str | None) -> str:
    ...
    明确账号记忆作用域，避免不同 Agent 会话之间污染画像记忆。
    """
...
def chat_turn_message(user_message: str, name: str | None) -> str:
    """构造发给 Ripple Agent 的一轮消息：画像前缀（如有）+ 用户原文 + 末尾行为提醒。

    末尾提醒对抗长对话里「忘记先查 SKILL」的指令衰减（见 TURN_REMINDER）。
    对用户不可见（前端只显示用户原文），只进入当前 Agent Runtime 上下文。
    """
