...
- WARN 级 = AI 措辞（由 AI 生成 / Claude / Anthropic / system prompt / 大模型）与
  模型名（例如 claude-* / gpt-image-2）→ 这些在论文解读、AI 科普、技术文章里
...
SECRET_PATTERNS: list[tuple[str, str, "re.Pattern[str]", str]] = [
    ...
    # ── 内部域名 / 服务地址 ──
    ("internal-host", "med", _p(r"\b[a-z0-9-]+(?:\.[a-z0-9-]+)*\.(?:internal|corp|intranet)(?:\.[a-z0-9-]+)+\b", re.I),
     "疑似内部域名"),
    ...
    # ── 内部绝对路径 ──
    ("internal-path", "med", _p(r"/mnt/[A-Za-z0-9._/-]+"), "挂载数据盘绝对路径"),
    ("internal-path", "med", _p(r"(?:~|/root)?/\.(?:opencode|ripple-private)\S*"), "本机私有运行目录"),
    # ── env 变量名 ──
    ...
    ("env-name", "med", _p(r"\b(?:IMG_API_KEY[A-Z_]*|IMG_BASE_URL|DASHSCOPE_API_KEY|"
                           ...
                           r"RIPPLE_PROXY|RIPPLE_ROOT|RIPPLE_PROXY|RIPPLE_ROOT)\b"), "Ripple env 变量名"),
    # ── 模型 / 供应商内部名 ──
...
def load_env_literals(root: str | Path | None = None) -> set[str]:
    ...
    root 缺省依次探测：RIPPLE_ROOT（兼容 RIPPLE_ROOT）→ 从本文件向上找含 .env 的目录。
    找不到 .env 返回空集（selftest 不依赖 .env）。
    """
    candidates: list[Path] = []
    if root:
        candidates.append(Path(root))
    env_root = os.environ.get("RIPPLE_ROOT")
    if env_root:
...
def cmd_selftest(_a) -> int:
    ...
    block_positives = [
        ...
        ("internal host", "调用 https://maas.internal.example/openai 出图"),
        ("intranet host", "打开 tool.intranet.example/xxx"),
        ("proxy ip", "代理设成 10.0.0.10:3128 就行"),
        ("port 3128", "端口 :3128 是代理"),
        ("path mnt", "产物在 /mnt/private-workspace/dataset/xxx"),
        ("path profile", "登录态在 ~/.ripple-private/browser-profile"),
