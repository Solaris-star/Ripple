#!/usr/bin/env python3
"""Single source of truth for Ripple output paths."""
...
def _discover_root() -> Path:
    configured = (os.environ.get("RIPPLE_ROOT", "")).strip()
    if configured:
        return Path(configured).expanduser().resolve()
    for parent in (Path(__file__).resolve().parent, *Path(__file__).resolve().parents):
        outputs = parent / "outputs"
        if outputs.exists():
            # Resolve symlinked output workspaces back to the owning Ripple project.
            return outputs.resolve().parent
    raise RuntimeError("找不到 Ripple 项目根；请设置 RIPPLE_ROOT")
...
class OutputPathError(ValueError):
    """Raised when a path violates the Ripple outputs layout."""
...
def validate_project_dir(value: str | os.PathLike[str], *, create: bool = False) -> Path:
    ...
    marker = Path(value) / ".ripple-path-check"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="校验 Ripple outputs 路径规约")
