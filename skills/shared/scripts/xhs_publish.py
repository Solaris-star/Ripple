...
本脚本用 Playwright + 持久化登录态，headless 即可发布。确定性 IO 固化在脚本，
文案/策略仍由上层 LLM 决定。
...
def _diag_after_publish(page) -> str:
    ...
    截图默认不存（正常使用不截图）；排错时设 RIPPLE_PUBLISH_DEBUG=1 才会存失败截图。"""
    bits = []
    for sel in (".d-modal", "[role=dialog]", "[class*=modal]", "[class*=dialog]",
        ...
        try:
            ...
            pass
    shot = ""
    if os.environ.get("RIPPLE_PUBLISH_DEBUG"):
        try:
...
def cmd_login(a) -> int:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print(f"   用小红书 App 扫码登录。若走 Ripple Web UI，可在 outputs 里查看这张图。")
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="小红书发布（Playwright，headless 可用）",
