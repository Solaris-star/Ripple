...
env RIPPLE_CALENDAR_AUTORECORD=0 时，脚本侧自动记录被禁用（发布页由 web 自己记录，防重复）。
"""
...
def _find_root() -> Path:
    """定位项目根：优先 RIPPLE_ROOT；RIPPLE_ROOT 仅作旧 workspace 兼容。
    兼容源码目录和旧的拍平 shared/scripts 布局；找不到环境变量时用标记探测。"""
    env = os.environ.get("RIPPLE_ROOT")
    if env:
        return Path(env)
...
def _forward_publish_log(platform: str, title: str, url: str, ptype: str,
    ...
    candidates = [PROJECT_ROOT / "skills" / "ripple" / "skill-publish-log" / "scripts" / "log.py"]
    ...
    try:
        subprocess.run(cmd, capture_output=True, timeout=15,
                       env={**os.environ, "RIPPLE_CALENDAR_AUTORECORD": "0"})
    except Exception:
        pass  # 转发失败绝不影响主流程
...
def record_publish(platform: str, title: str, url: str = "", ptype: str = "",
    ...
    env RIPPLE_CALENDAR_AUTORECORD=0 时直接跳过（发布页由 web 记录，防重复）。"""
    if os.environ.get("RIPPLE_CALENDAR_AUTORECORD") == "0":
        return False
    try:
...
def cmd_record_publish(args) -> None:
    ...
    print(json.dumps({"ok": ok, "skipped": not ok and
                      os.environ.get("RIPPLE_CALENDAR_AUTORECORD") == "0"},
                     ensure_ascii=False))
...
def _locate_lunar():
    for c in (PROJECT_ROOT / "skills" / "ripple" / "skill-event-calendar" / "scripts" / "lunar.py",):
        if c.is_file():
            return c
    return None
...
def cmd_selftest(_a) -> None:
    ...
    try:
        ...
        # AUTORECORD=0 应跳过
        os.environ["RIPPLE_CALENDAR_AUTORECORD"] = "0"
        assert record_publish("douyin", "不该写", data_path=data, forward_log=False) is False
        del os.environ["RIPPLE_CALENDAR_AUTORECORD"]
