...
Provider（--provider 或 env VIDEO_PROVIDER）：
  ...
  xhs-maas           旧版兼容标识：异步 media/parameters 协议。公开版不提供默认端点/模型；
                     仅用于迁移已有本地配置。
...
# Legacy async media/parameters adapter. Endpoint is user-supplied; Ripple does not
# ship an organization-specific MaaS hostname in the public distribution.
DEFAULT_XHS_MAAS_VIDEO_BASE = ""
...
PROVIDER_CAPABILITIES: dict[str, dict[str, Any]] = {
    ...
    # Legacy compatibility adapter: capabilities are unknown until explicitly overridden.
    "xhs-maas": {"native_audio": False, "dialogue": False, "dialogue_faithful": False, "audio_reference": False,
                 "audio_default": False},
...
PROBE_CACHE_PATH = Path.home() / ".ripple-video-capabilities.json"
...
def _managed_network() -> str | None:
    if os.environ.get("RIPPLE_MANAGED_MEDIA", "").strip().lower() not in ("1", "true", "yes", "on"):
        return None
    return os.environ.get("RIPPLE_MEDIA_NETWORK", "system").strip().lower() or "system"
...
class _HttpxResponseAdapter:
    ...
    def __exit__(self, *_args):
        self.response.close(); self.client.close(); return False
    def read(self) -> bytes: return self.response.content


def _open_request(req: urllib.request.Request, timeout: int, direct: bool = False):
    managed = _managed_network()
    if managed == "custom":
        proxy = os.environ.get("RIPPLE_MEDIA_PROXY", "").strip()
        if not proxy:
            raise urllib.error.URLError("custom proxy is not configured")
        client = httpx.Client(proxy=proxy, trust_env=False, timeout=timeout, follow_redirects=True)
        try:
            response = client.request(req.get_method(), req.full_url, headers=dict(req.header_items()), content=req.data)
        except httpx.HTTPError as exc:
            client.close(); raise urllib.error.URLError(str(exc)) from None
        if response.status_code >= 400:
            body = response.content; status = response.status_code; reason = response.reason_phrase; response_headers = response.headers
            response.close(); client.close()
            raise urllib.error.HTTPError(req.full_url, status, reason, response_headers, io.BytesIO(body))
        return _HttpxResponseAdapter(client, response)
    if managed == "direct":
...
def http_request(url: str, headers: dict[str, str], method: str = "GET",
    ...
    try:
        with _open_request(req, timeout, direct=direct) as resp:
...
# ── provider: xhs-maas（旧版兼容 ID：异步 media/parameters 协议）─────
def generate_xhs_maas(args: argparse.Namespace, image: str | None) -> Path:
    """Legacy compatibility adapter retained for existing local configuration.

    Public Ripple ships no endpoint, model, organization hostname, or capability assumptions for it.
    """
    api_key = require_env("XHS_MAAS_API_KEY")
    base = (os.environ.get("XHS_MAAS_VIDEO_BASE", "").strip()
            or DEFAULT_XHS_MAAS_VIDEO_BASE).rstrip("/")
    if not base:
        fail("xhs-maas 兼容协议需要显式配置 XHS_MAAS_VIDEO_BASE；公开版不提供组织专用默认端点。")
    model = resolve_model("xhs-maas", args.model, image=image is not None)
    if not model:
        fail("xhs-maas 兼容协议需要显式配置模型；公开版不提供组织专用默认模型。")
    if image:
