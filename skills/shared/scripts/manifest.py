...
Ripple 纵向编排（发现/策划/制作/发布/归因）跨层时，上游产物路径与关键结论
过去只靠 agent「凝练」口传，无结构化传递。本脚本把每层步骤登记到
`outputs/<主题>/.ripple.json`，让下游用 `latest` 直接取上游结论/产物，
...
Schema:
  {
    ...
    "content_id": "可选：Ripple Mother content UUID",
    "source_version_id": "可选：生成时对应的 Mother version_id",
...
**目录布局规约**：一个产物目录 = `outputs/<主题>/`；内容工作台的 Mother content
是产品层归档单位。成品（用户要发/读的最终文件）放目录根，中间件（frames/clips/构建脚本/原始素材/草稿/重复文件）进
`assets/` 子目录；`.ripple.json` 是该产物目录唯一元数据（隐藏）。系统状态一律
`_` 前缀目录（_login/_publish/_analytics/_scratch…）。

子命令:
  record   追加一步（新主题自动初始化；--status failed 记失败步）
  meta     upsert 展示头（title/platform/kind/status/tags/cover/deliverables/content_id/source_version_id）
  read     打印整份 manifest
  latest   取最近一步（可按 --layer 过滤），供下游读上游

数据文件默认 <Ripple 根>/outputs/<主题>/.ripple.json，可用 --data 覆盖。
"""
...
# RIPPLE_ROOT 为公开配置名；RIPPLE_ROOT 仅保留旧 workspace 兼容。
# 兼容同步产生的拍平副本仍需要环境变量固定项目根。
PROJECT_ROOT = Path(os.environ.get("RIPPLE_ROOT") or Path(__file__).resolve().parents[3])
OUTPUTS_DIR = PROJECT_ROOT / "outputs"
MANIFEST_NAME = ".ripple.json"
...
META_SCALAR_FIELDS = ("title", "summary", "platform", "kind", "status", "cover", "content_id", "source_version_id")
...
def manifest_path(topic: str | None, data_override: str | None) -> Path:
    """--data 优先；否则按主题落到 outputs/<主题>/.ripple.json。"""
    if data_override:
...
def cmd_meta(args) -> None:
    ...
    header = {k: data.get(k) for k in
              ("title", "summary", "platform", "kind", "status", "tags", "cover", "deliverables", "content_id", "source_version_id")
              if k in data}
...
def _selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        mp2 = str(Path(td) / "sub" / ".ripple.json")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    m.add_argument("--content-id", dest="content_id", help="关联的 Ripple Mother content UUID")
    m.add_argument("--source-version-id", dest="source_version_id", help="生成时对应的 Mother version_id")
