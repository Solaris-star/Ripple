#!/usr/bin/env python3
"""ai_image.py — 通用 AI 文生图 / 图生图 / 图像变体。

复用 Ripple 已验证的 ecom-details-image/scripts/generate_image.py 约定：
...
def _encode_multipart(fields: dict[str, str],
    ...
    boundary = f"----RippleBoundary{int(time.time() * 1000)}"
    buf = bytearray()
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="通用 AI 文生图 / 图生图 / 图像变体（OpenAI 兼容同步 + 异步任务）。",
