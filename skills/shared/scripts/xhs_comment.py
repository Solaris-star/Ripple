...
_COUNT_COMMENT_JS = """([nickname, content]) => {
    const authors = Array.from(document.querySelectorAll('.author')).filter(a => a.textContent.trim() === nickname);
    let count = 0;
    for (const a of authors) {
        let c = a.parentElement;
        for (let i = 0; i < 6 && c; i++) {
            if (!content || (c.textContent || '').includes(content)) { count += 1; break; }
            c = c.parentElement;
        }
    }
    return count;
...
def _profile_dir(base: str | None) -> Path:
    if base:
        ...
        root = Path.home() / ".ripple-browser-profiles"
...
def _proxy(explicit: str | None, disable: bool) -> str | None:
    ...
    return os.environ.get("https_proxy") or os.environ.get("http_proxy") \
        or os.environ.get("RIPPLE_PROXY")


def _debug_on() -> bool:
    return bool(os.environ.get("RIPPLE_COMMENT_DEBUG"))


def _shot(page, name: str) -> None:
    """排错截图：仅 RIPPLE_COMMENT_DEBUG=1 时存到 outputs/_login/comment-*.png。"""
    if not _debug_on():
        return
    try:
...
def _parse_replies(raw: str | None) -> list[dict]:
    ...
    for i, r in enumerate(data):
        ...
        out.append({"id": str(r.get("id") or ""), "nickname": str(r["nickname"]),
                    "content": str(r.get("content") or ""), "reply": str(r["reply"])})
    return out
...
def _do_reply(page, nickname: str, text: str, content: str = "") -> bool:
    """用昵称+原评论片段唯一定位目标，再发送回复。无法唯一定位时拒绝发送。"""
    count = int(page.evaluate(_COUNT_COMMENT_JS, [nickname, content]) or 0)
    if count != 1:
        reason = "未找到目标评论" if count == 0 else "存在多个同名/同内容候选"
        print(f"  ❌ @{nickname}: {reason}，未发送；请带原评论 content 重新定位", file=sys.stderr)
        return False
    handle = page.evaluate_handle(_FIND_COMMENT_JS, [nickname, content])
    container = handle.as_element()
    if not container:
        print(f"  ❌ @{nickname}: 未找到目标评论容器，未发送", file=sys.stderr)
        return False
    btn = None
    for el in container.query_selector_all('*'):
        try:
            if (el.text_content() or '').strip() == '回复' and el.is_visible():
                btn = el; break
        except Exception:
            continue
    if not btn:
        print(f"  ❌ @{nickname}: 未找到回复按钮，未发送", file=sys.stderr)
        return False
    ...
    try:
        ...
        inp.press("Control+A"); inp.press("Backspace"); inp.type(text)
...
def _do_delete(page, nickname: str, content: str = "") -> bool:
    ...
    if not page.evaluate(_CLICK_MENU_ITEM_JS, list(DELETE_MENU_TEXTS)):
        print(f"  ❌ @{nickname}: 未找到删除菜单项（DOM 变化，RIPPLE_COMMENT_DEBUG=1 存 DOM 校准）",
              file=sys.stderr)
        return False
    page.wait_for_timeout(800)
...
def cmd_notes(a) -> int:
    ...
    if miss:
        print(f"⚠️ {len(miss)}/{len(notes)} 篇未取到 xsec_token——抓这些的评论用 `fetch --url '<该笔记分享/打开链接>'`；"
              "或 RIPPLE_COMMENT_DEBUG=1 + --headed 看接口是否改版。", file=sys.stderr)
    return 0
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for i, r in enumerate(todo):
                if _do_reply(page, r["nickname"], r["reply"], r.get("content", "")):
                    ok_n += 1
                    if r["id"]:
                        replied.add(r["id"])
...
def main() -> int:
    ...
    def add_common(p):
        p.add_argument("--profile-base", help="登录态根目录（默认 ~/.ripple-browser-profiles；自动兼容旧目录）")
