...
**cast.json**（选角示例；引擎按角色需要、当前可用能力和用户质量要求选择）：
```json
{"cast":[
...
- **建议**：关键角色可优先选择满足情感/专属音色要求的引擎；`cast check` 应关注说话人映射、缺失音色、重复/不合理绑定等客观问题。若当前脚本仍因为已配置某类 Provider 而硬拦其他引擎，这是实现层限制，规则层不把它提升为通用质量标准。
...
```bash
python skills/ripple/short-drama/scripts/dubbing.py cast init --series "<剧名>"
