...
_DEFAULT_PROXY = os.environ.get("RIPPLE_PROXY", "")  # lab 在 .env 设 RIPPLE_PROXY；不设则不走代理
...
def _post_json(url: str, payload: dict, timeout: int = 20) -> tuple[int, str]:
    ...
    req = urllib.request.Request(
        url, data=data,
        headers={"Content-Type": "application/json", "User-Agent": "Ripple-notify/0.1"},
        method="POST")
    try:
...
def cmd_selftest(_a) -> int:
    ...
    try:
        ...
        status, body = _post_json("https://httpbin.org/post",
                                  build_payload("generic", "ripple-selftest", None),
                                  timeout=12)
        if status == 200 and "ripple-selftest" in body:
