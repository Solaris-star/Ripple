...
| 画幅 | 需要确定 | 优先复用用户明确值、已有项目规格或目标内容的确定比例；只有多个合理选择会明显改变结果时才询问，可逆预览可用说明过的默认 |
...
- 有 Profile：需要平台规格或视觉偏好时按需读取；已有画幅直接复用，卡点风格根据账号调性建议。
- 无 Profile：画幅没有关键歧义时采用任务/平台合理默认并说明；默认每 2 拍、zoom 特效可随时调整。
