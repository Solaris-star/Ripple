...
- 首次 `login` 扫码/登录快手创作者中心，登录态持久化到 `~/.ripple-browser-profiles/KuaishouProfile` 后复用；已有旧目录会自动兼容
...
- 仍出现「显示已登录、发布却失败」= 发布态过期：去 Web「账号」页重新扫码，或清空
  `~/.ripple-browser-profiles/KuaishouProfile` 后重跑 `login-qr`（若本机仍在兼容旧目录，以脚本实际检测路径为准）。
