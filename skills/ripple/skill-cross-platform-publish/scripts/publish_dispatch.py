...
def cmd_selftest(_a) -> int:
    ...
    # 合规内容无告警
    ok = _check_platform("kuaishou", {"title": "短标题", "tags": ["a", "b"],
                                      "media_type": "video"})
    ...
    m = {"content": {"title": "测试", "body": "正文", "tags": ["x"], "media_type": "video"},
         "platforms": ["bilibili", "kuaishou", "weixin-channels"]}
    import tempfile
    from pathlib import Path
    with tempfile.TemporaryDirectory() as td:
        ...
        assert plan[0]["publisher"] == "skill-bilibili-upload"
    print("OK: selftest 通过（约束检查/告警/路由/plan）")
    return 0
