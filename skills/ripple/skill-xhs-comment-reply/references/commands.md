...
本文件仅供独立 CLI/维护排错；Ripple 日常互动优先使用「互动管理」，以获得账号隔离、任务记录和 unknown-result 防重复保护。
...
回复列表格式 `[{id, nickname, content, reply}]`。`content` 保留原评论片段用于同名去歧义；托管路径优先使用评论 ID，CLI 无法唯一定位时应拒绝发送：
...
python skills/shared/scripts/xhs_comment.py reply \
  --note-id <id> --xsec-token '<token>' --no-proxy \
  --replies-json '[
    {"id":"6a6abc09...","nickname":"用户A","content":"怎么开始？","reply":"可以先从第一步开始。"},
    {"id":"6a6abbcf...","nickname":"用户B","content":"参数是多少？","reply":"这里的关键参数是……"}
  ]'
...
- `--gap`：CLI 批处理的固定节奏参数，不保证规避平台限制；遇到验证码/限制应停止，而不是靠调整间隔穿过风控。
- 首次或疑似改版：加 `--headed` 观察一遍再 headless 批量。
- 排错：`RIPPLE_COMMENT_DEBUG=1 python ... reply ...` 会把点击后/输入后截图存到
  `outputs/_login/comment-*.png`。

## 2b. 删除评论 delete（默认 dry-run，确认后 --exec；不可恢复）

删自己笔记下的评论（含自己发的回复）。优先使用评论 ID；CLI 的昵称定位至少配合 `--content` 片段保证唯一候选，无法唯一定位就不删。
**全程文本/结构定位，无坐标。**
...
- 排错：`RIPPLE_COMMENT_DEBUG=1` 会把菜单/确认弹窗截图 + DOM 存到 `outputs/_login/`，据此校准
...
python skills/ripple/skill-comment-insights/scripts/comment_insights.py \
...
python skills/ripple/skill-publish-log/scripts/log.py record --platform 小红书 \
