...
3. **调用脚本校验**：`python3 skills/shared/scripts/wordcount.py check --target <N> -f <文件>`（或经 stdin 传入）
   ...
   - `±5%` 是“目标篇幅”的默认容差示例，不等于平台硬上限。平台规定“最多 N”时完成标准是 `social_count <= N`；用户要求接近某个目标值时才使用双向 tolerance。
4. 未达标就根据脚本给出的差值定向调整并重查；每轮应缩小偏差。若保留项与目标长度冲突，或连续两轮没有改善，停止并报告冲突，不做无界循环。
5. 确认无断句、无残句
