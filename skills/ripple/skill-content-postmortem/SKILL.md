...
已连接小红书账号可从 Ripple「发布记录 → 小红书作品表现」或 `ripple_xhs_read(operation=notes/note/comments)` 使用近期作品的可见指标。`views/likes/collects/comments/shares` 缺失时保持 unknown，不补 0；发布任务关联优先使用作品 ID/作品链接，标题只能作为弱兜底。

### 模式 A — 单条复盘

1. **确认模式**：根据用户输入判断是单条复盘还是规律提炼。若用户只提供一条内容，进入模式 A。
2. **采集上下文**：确认平台、发布时间和实际可得指标。小红书已连接账号优先复用 Ripple 当前作品快照；缺失指标标 unknown，只有核心判断依赖它时才询问用户补充。
3. **基线建立**：优先使用同一账号、相近内容形态的同期样本做相对比较；没有可比样本时就做定性复盘，不用“平台通用基线”假装是当前账号事实。
4. **多维拆解**：可打分维度（Hook力/内容结构/信息密度/互动引导/视觉排版/平台适配）用 `references/postmortem-dimensions.md` 的 1-10 标尺；选题、时间节奏做定性判断。每个维度给出判断和证据。
5. **归因排序**：识别最值得验证的 1-2 个因素，并把“观察到的相关性”和“可能原因”分开。平台推荐、热点窗口、发布时间等不可见变量不能被当作已证实因果。
...
2. **调用脚本聚合**：
   ```bash
   python3 skills/ripple/skill-content-postmortem/scripts/aggregate.py --input contents.json
   python3 skills/ripple/skill-content-postmortem/scripts/aggregate.py --input contents.json \
...
3. **模式识别 + 候选公式（LLM 解读）**：从 `by_dimension`/`cross` 读出高频、高 lift 的组合，总结为“待验证模式”（选题 + Hook + 结构）。单次/小样本结果不直接命名为稳定爆款公式；跨后续作品重复验证后再提高信心。
