...
> 格式借鉴 Seedance2-Storyboard-Generator 的时间轴写法，适配 Ripple 的 ai-image-gen / ai-video-gen。
...
- 因此生视频**必须传 `generation_prompt`**（prepare 写的、含台词契约的那个），**不是 `prompt`**。逐字对白能力必须以当前所选模型的探针结果为准；没有探针证据时按“不保证逐字忠实”处理。
