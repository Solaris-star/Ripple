...
def _selftest() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            cmd_generate(td, "测试剧", 1, None, False, "xhs-maas", "fixture-i2v-model",
                         "9:16", dry_run=True)
            gate_fired = False
        except SystemExit:
            gate_fired = True
        ...
        _write_json(epg / "shots.json", {"episode": 1, "style_prefix": "港风",
                    "video_generation": {"provider": "xhs-maas", "model": "fixture-i2v-model"},
                    "shots": [{"idx": 1, "prompt": "画面", "refs": ["C01"], "frame": "shots/f1.png",
        ...
        try:
            rc = cmd_generate(td, "测试剧", 1, None, False, "xhs-maas", "fixture-i2v-model",
                              "9:16", dry_run=True)
        except SystemExit:
            rc = 99
