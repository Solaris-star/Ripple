...
```
bible/    world.md characters.md voice.md canon.md   ← 长期设定（按本章涉及内容读取）
...
脚本路径（相对项目根）：`skills/ripple/novel-writer/scripts/novel_ops.py`。
...
1. **搭骨架**：`python skills/ripple/novel-writer/scripts/novel_ops.py scaffold --book "<书名>"`
...
5. **只加载相关切片**（不塞全书）：读本章大纲、`state/summary.md`、相关角色状态，以及本章涉及的 world/characters/voice/canon 条目。新卷、全局设定调整或发现冲突时再扩大到完整 bible；同一任务已读取且未变化的设定不重复加载。
...
8. **篇幅检查（有明确目标时）**：平台或用户给了目标字数才用 `wordcount.py` 检查；平台硬上限按 `<= N` 判断，建议篇幅只作参考，不为了命中区间填充无价值内容。
9. **自然度检查（按需）**：用户要求去套话、当前章节出现明显重复模板时才运行 `slopcheck`/text-polisher；设定一致性、因果和角色行为优先于主观“AI 味”分数，同一未变化章节不重复全量扫描。
...
（场景卡分解替代长上下文）——均为「文件即真相 + 按需加载」思路，最适合纯文件工具的 Ripple。
