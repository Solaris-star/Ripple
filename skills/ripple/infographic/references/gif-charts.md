...
> 本文件说明 4 个子命令的适用场景与数据格式。自检：`python skills/ripple/infographic/scripts/gif_chart.py --selftest`。
...
```bash
# 条形竞赛，前 8 名，6 秒
python skills/ripple/infographic/scripts/gif_chart.py bar-race --data data.json --title "平台粉丝榜" --top-n 8 --duration 6 -o outputs/主题名/chart.gif

# KPI 数字滚动（stdin 传数据）
echo '{"items":[{"label":"累计GMV","value":8600000,"prefix":"¥"}]}' | python skills/ripple/infographic/scripts/gif_chart.py count-up --data -

# 环形进度
python skills/ripple/infographic/scripts/gif_chart.py progress --data prog.json --style ring
```
