...
- **模式 B（动画 GIF）** → 调 `scripts/gif_chart.py <子命令>`，无需手写动画代码。子命令按图型：
  ...
  ```bash
  python skills/ripple/infographic/scripts/gif_chart.py bar-race --data data.json \
    --output outputs/城市增长/柱状竞速.gif --width 900 --duration 5
  ```
  脚本自动设置中文字体、Agg 后端、自适应调色板控体积。自检：`python skills/ripple/infographic/scripts/gif_chart.py --selftest`。
