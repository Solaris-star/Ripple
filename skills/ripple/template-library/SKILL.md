...
> **Ripple 托管入口（Phase 2）**：内容创作和 Agent 的“套用”统一调用结构化操作 `template_apply`。它只接受模板 ID 与声明过的变量，先返回缺失变量和正文预览；预览不会增加使用次数，也不会写回正文。只有用户显式应用后，前端才把预览复制到当前草稿。保存/编辑/删除模板仍属于独立模板管理能力，不与这条预览操作混为一体。
...
4. 统一入口（默认 `--root templates`）：
   ```bash
   S=skills/ripple/template-library/scripts/templates.py
