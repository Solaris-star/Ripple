...
该可选流程需要用户自行安装 Wechatsync Chrome 扩展与 CLI，并复用扩展中的平台登录状态。
...
5. 自检:
   ```bash
   python3 skills/ripple/skill-wechat-publisher/scripts/multi_publish.py --check
   ```
   两项都打 `✓` 说明就绪。
...
**方式 A:命令行显式指定平台(最常用)**
```bash
python3 skills/ripple/skill-wechat-publisher/scripts/publish.py --account main \
...
```bash
python3 skills/ripple/skill-wechat-publisher/scripts/publish.py --account main --input x.md --cover x.jpg --sync-from-config --exec
```

**方式 C:独立跑(不发微信,只同步)**
```bash
python3 skills/ripple/skill-wechat-publisher/scripts/multi_publish.py --input x.md --platforms zhihu,juejin --exec
```
