...
白底 + 粗刷体大字(干笔飞白)+ 荧光笔高亮 + 单个关键词强调色，是适合短观点/金句的一种可选视觉方向。是否作为默认由当前账号/Profile/用户参考决定，不绑定仓库示例账号。

**共享视觉特征（参考）**：字号有层级、刷体大字保持足够行距、文字块保留呼吸、高亮和强调色克制使用。这些是该风格的识别特征，具体比例随文案长度、画幅和品牌规范调整，不作为所有卡片的硬门禁。
...
适合作为技术短观点/爆点卡的候选风格；是否默认由当前用户配置决定。
