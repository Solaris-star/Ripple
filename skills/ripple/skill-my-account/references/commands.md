...
- 身份：`skills/shared/scripts/{xhs_publish,web_publisher}.py whoami`
...
> 抖音仅保留已有本地 Browser Profile 的只读数据兼容；首个公开版本不提供抖音登录/发布器。
输出 JSON（关键字段）：
...
```bash
RIPPLE_STATS_DEBUG=1 python skills/shared/scripts/account_stats.py fetch --platform <平台>
