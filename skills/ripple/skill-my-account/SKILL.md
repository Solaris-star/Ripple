...
description: |
  查询用户自己在 Ripple 已登录的小红书、抖音、快手、知乎和视频号身份、粉丝、获赞、关注及作品列表。
...
**Ripple 托管小红书**：优先使用 `ripple_xhs_read(operation=accounts/notes)` 读取已连接账号和近期作品，不读取浏览器 Cookie/路径，也不要求用户重新提供主页链接。其他平台或独立维护环境才按原有确定性脚本能力处理。

统一走确定性脚本（CWD=项目根）。读的是**本地登录态**（新默认 `~/.ripple-browser-profiles/<平台>Profile`；已有旧目录自动兼容），
...
- **评论**：小红书可通过 `ripple_xhs_read(operation=comments)` 读取；评论量化分析交 **skill-comment-insights**，拟稿/真实互动编排交 **skill-xhs-comment-reply**。

## 支持平台

whoami：小红书 / 知乎 / 快手 / 视频号。
...
> **视频号**登录后按创作页取。抖音数据抓取只保留给已有本地 Browser Profile 的兼容场景；首个公开版本不内置抖音登录/发布器。拿不到的项如实显示"—"，不编不凑。
...
- **skill-xhs-analyzer** = 小红书**推荐流/关键词/单篇/账号作品/评论的证据式分析**。
...
- 未登录不报错收场，明确提示去哪登录。
- 小红书作品对外只给脱敏 public URL，不回显 xsec token 等短期定位参数；其他平台给可安全公开的作品链接。
- 平台改版导致抓取字段为空时，如实说"这项这次没取到"，可 `RIPPLE_STATS_DEBUG=1` dump 正文排查（见 commands）。
