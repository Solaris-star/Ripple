...
联网遵循 Ripple 代理约定：先读 http(s)_proxy，其次 RIPPLE_PROXY，都无则直连。
...
def fetch(paper: str, out: str) -> str:
    ...
    url = info["pdf_url"]
    dst = _prep_out(out)
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 Ripple"})
    try:
        with _opener().open(req, timeout=60) as r:
            data = r.read()
    except Exception as e:
        _die(f"下载失败：{url}\n  {e}\n  （需外网？检查 http(s)_proxy / RIPPLE_PROXY）")
    if not data[:5] == b"%PDF-":
...
def _mineru_rest(pdf: Path, out_dir: Path, token: str) -> dict:
    ...
    entry = {"name": pdf.name, "is_ocr": False,
             "data_id": "ripple_" + re.sub(r"[^0-9A-Za-z_.-]", "_", pdf.stem)[:64]}
...
def _selftest() -> int:
    ...
    os.environ.pop("HTTP_PROXY", None)
    os.environ.pop("RIPPLE_PROXY", None)
    chk("无代理时直连", _proxies() == {})
    os.environ["RIPPLE_PROXY"] = "http://x:3128"
    chk("RIPPLE_PROXY 生效", _proxies().get("https") == "http://x:3128")
    os.environ.pop("RIPPLE_PROXY", None)
