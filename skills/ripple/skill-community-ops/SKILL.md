...
小红书已连接账号可以先用 `skill-xhs-comment-reply` / `ripple_xhs_read` 读取真实评论；用户已经贴出评论时直接使用，不重复抓取。模式 A 默认只产出可编辑草稿；用户明确要求执行时，再由 `skill-xhs-comment-reply` 建立互动草稿并交「互动管理」确认。

1. 读 `references/reply-playbook.md`「一、五类评论分层话术库」，按五类归类用户给的评论：普通赞美 / 专业提问 / 求购买求链接 / 杠精抬杠 / 黑粉恶意差评。
2. 对实际评论优先给一条贴合上下文的具体回复；用户要话术库时才为每类给 2-3 条模板。模板中的承诺、链接、后续内容计划只有在真实存在/已确认时才能填入。
...
1. 使用本次已有评论样本，按 `references/topic-mining.md` 聚类高频诉求、重复疑问、争议点、许愿和吐槽；已有分析结果可复用，不为了这个模式再次读取同一批评论。
...
6. 不为了互动率编造“下期一定出”“资料已整理”“已经处理”等承诺；只有事实已经成立或用户明确承诺后才写。
7. 小红书真实回复/评论/删除必须经过 Ripple 互动任务确认；本 Skill 的文案策略不等于外部执行授权。
