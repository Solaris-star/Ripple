...
Ripple Settings 可以保存语音 Provider/Model，但托管 Agent 只有在当前会话实际暴露对应的受控语音/克隆工具时才执行 enroll/clone；Skill 本身不授予上传样本、调用第三方语音服务或读取 Key 的权限。没有受控工具时可准备文案、样本要求和参数，不声称已完成克隆。

独立 CLI/维护场景才按下列 provider 配置运行 `voice_clone.py check`：
```bash
...
托管调用若存在对应工具，优先使用用户本次指定，其次使用 Settings 默认 Provider+Model；多个可用不需要重复询问。独立 CLI 才按自身参数/本地配置选择 provider。
...
2. 托管调用由后端 capability/错误负责配置判断；独立 CLI 才按 `check → enroll → clone` 顺序执行，避免重复检查。
