...
| 画幅 | 需要确定 | 优先复用用户明确值、已有项目规格或目标内容的确定比例；可逆预览可用说明过的合理默认，只有多个合理选择会明显改变最终交付时才询问 |
| 每图时长 | 否 | 默认每张 3 秒 |
...
- 有 Profile：按需读取平台规格与 style；已有画幅直接复用，BGM/字幕风格贴合账号调性。
- 无 Profile：根据素材和目标平台采用合理默认；只有画幅选择确实影响最终交付时再询问。转场默认 fade + Ken Burns。
