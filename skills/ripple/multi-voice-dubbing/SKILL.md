...
不同引擎的情感控制、音色和稳定性不同；质量应以当前可用模型、试听结果和角色需求判断。需要强情感控制或专属音色时可优先选择支持相应能力的云 Provider，但不把 Provider 类别本身当作“像人/不像人”的硬门：

| 引擎（cast 里 `engine`）| 质量 | 需要 | 情感机制 |
|---|---|---|---|
| `edge` | 公共预置音色，能力相对基础；是否适合成品看任务与试听 | 无专用 key、需联网 | rate/pitch/volume 微调 |
...
独立 CLI 可以配置支持情感/克隆能力的云 Provider；下面只作示例，不代表当前 Ripple 默认或长期推荐：
```bash
...
- 主角/关键角色需要更强情感或专属音色时，选择具备对应能力的 Provider；若用户明确接受公共音色或试听结果达标，不额外强制云 Provider。
- Ripple 托管调用以 Settings/capability 为准；独立 CLI 才按 `voice_clone.py` 的本地配置方式检查 Provider。
...
新建 cast、角色声音要求变化或出现声线错配时读取 `skills/shared/references/voice-casting.md`；同一项目已有稳定 cast 的局部台词修改直接复用，不重新选角。
...
1. **多人对话要能区分说话人**：通常为主要角色分配不同音色；用户明确选择同一声线表演、角色很少或特殊艺术处理时可例外，但 speaker 映射必须正确。
2. **引擎按目标与实测选择**：需要情感/克隆能力时用支持该能力的 Provider；公共音色若满足用户要求也可用于成品。
