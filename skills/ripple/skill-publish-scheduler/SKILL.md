...
> （skill-xhs-publisher / skill-kuaishou-upload / skill-wechat-publisher / skill-bilibili-upload / cross-platform-publish）。
...
2026-08-01 20:00,kuaishou,outputs/主题名/final.mp4,晚间视频
```
或等价 JSON。

## 执行

脚本路径（相对项目根）：`skills/ripple/skill-publish-scheduler/scripts/publish_queue.py`（各子命令 `-h`）。
...
4. 需要"守护式到点触发"时，由外部调度器或 Ripple 周期任务运行 `run`（本脚本只做单次到期计算，不常驻）。
...
5. 定时触发依赖上层调度器（系统 cron / 手动周期跑均可），本脚本是无状态到期计算 + 持久化队列。
