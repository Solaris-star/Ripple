...
优先使用用户已有 Profile、当前回答和真实写作样本。3-5 篇样本能提高稳定性，但不是启动分析的绝对门槛；1-2 篇时输出“临时画像 + 证据覆盖不足”。仓库示例只能演示格式，不能当作用户自己的声音证据。
...
仅仅加载/审查本 Skill 不启动访谈。用户明确要构建画像时，先复用 Profile 和本轮已经给出的答案，直接问第一个真正缺失的问题；不问“准备好了吗”，也不重复询问已有信息。
...
- 3-5 篇用于形成更稳定的跨样本模式；不足 3 篇时仍可分析，但明确低置信度和未覆盖特征。
- `references/sample-content.md` 只用于演示方法或测试输出结构，不用于推断用户真实文风。
...
先交付画像结果。只有用户明确同意写入长期 Profile 时才合并到对应 `style.md`/相关画像文件；否则把它作为本次任务产物，不声称后续会自动引用。
...
1. **任务触发** — 只有用户明确要构建/更新声音画像才进入访谈；审查或咨询不执行
2. **样本覆盖** — 3-5 篇更可靠；样本较少时允许产出临时画像并标置信心，不伪造不存在的模式
