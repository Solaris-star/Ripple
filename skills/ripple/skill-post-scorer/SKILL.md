...
用户明确要求评分/发布前评估时直接评分，不额外确认“是否开始”。审查、解释或比较本 Skill 时只读规则，不对示例/文件自动打分。
...
```bash
python3 skills/ripple/skill-post-scorer/scripts/score.py top --input history.json
python3 skills/ripple/skill-post-scorer/scripts/score.py top --input history.json --top-pct 5
```
