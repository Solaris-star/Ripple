...
> **Ripple 托管入口**：互动管理和 Agent 统一调用结构化操作 `comment_analysis`，输入必须是 Ripple 从真实平台账号同步得到的评论记录，并复用互动中心同一分析器。产品主流程不接受手工 TXT/JSON/CSV 导入来模拟平台评论；下面的文件脚本仅保留给独立 CLI 分析场景。
...
脚本路径（相对项目根）：`skills/ripple/skill-comment-insights/scripts/comment_insights.py`。
