...
Ripple 的工作流分为多个层级（发现层、策划层、制作层、发布层、归因层）。每个 Profile 文件在不同层级中扮演不同角色：

| 文件 | 内容 | 使用层级 |
|------|------|----------|
| `identity.md` | 账号定位、差异化、内容方向 | Ripple Agent 发现层 + 策划层（定位参考） |
...
定义账号的核心定位和差异化。Ripple Agent 的发现层用它来筛选匹配的热点和素材，策划层用它来判断选题是否契合账号方向。制作层不直接注入此文件——定位已经通过选题间接传递。
