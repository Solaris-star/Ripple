...
覆盖 Ripple Web 后端、Skill 路由、画像、输出树与媒体辅助逻辑。
运行：pytest tests/ -q
"""
...
sys.path.insert(0, str(PROJECT_ROOT / "skills" / "ripple" / "paper-explainer" / "scripts"))

from ripple import persona  # noqa: E402
...
def test_persona_skill_prioritizes_positioning_and_caps_cross_niche_scores():
    skill = (PROJECT_ROOT / "skills/ripple/skill-persona-check/SKILL.md").read_text(encoding="utf-8")
...
def test_output_tree_relative_paths(tmp_path, monkeypatch):
    ...
    legacy = next(g for g in tree if g.get("legacy_unlinked"))
    grp = next(g for g in legacy["children"] if g["name"] == "主题A")
    # 无可靠 content_id 的旧目录进入历史未关联区；内部相对路径保持不变。
...
def test_last_turn_rejects_stale_turn(tmp_path, monkeypatch):
    ...
    artifact = {"kind": "content_draft", "id": "a" * 32, "version_id": "b" * 64, "title": "恢复草稿", "status": "draft"}
    web._save_turn("web:session-a", "done", "上一轮回答", {"turn_id": "old-turn", "artifacts": [artifact]})
    ...
    assert current["artifacts"] == [artifact]
...
def test_chat_stop_interrupts_mapped_agent_runtime_session(monkeypatch):
    ...
    assert calls == [("stop-cleanup-test", 5.0, "opencode")]
