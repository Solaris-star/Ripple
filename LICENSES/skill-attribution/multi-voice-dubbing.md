...
| **所属层** | produce |
| **来源类型** | 自研 |
| **原始来源** | Ripple 自研。核心引擎 `skills/shared/scripts/multivoice.py`：cast（选角）+ lines（逐行对白）→ 逐行按角色音色合成 → ffmpeg 拼多声线 voice.mp3 + 说话人字幕 voice.srt；情绪→韵律映射自带；纯标准库 + selftest |
...
| **许可** | 随 Ripple 项目许可；底层 edge-tts GPL-3.0 / FFmpeg LGPL-GPL |
