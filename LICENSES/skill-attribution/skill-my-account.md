...
| **实现** | `../../shared/scripts/account_stats.py`（数据）+ `xhs_publish.py`/`web_publisher.py` 的 `whoami`（身份），均 Playwright、headless 可用 |
| **自研** | ✅ Ripple 自研 |
| **自包含** | 需对应平台登录态（新默认 `~/.ripple-browser-profiles/<平台>Profile`；存在旧目录时自动兼容）；不依赖画像 |
...
但**只有 Web 后端 `/api/analytics` 在用**；相关 publisher 的 `whoami` 也只被账号页调用。此前 Agent
**没有指向这些能力的 SKILL 入口**——用户问"我小红书最近发了什么帖子"，agent 因为不知道自己能查
