...
let activeBrowserScope = 'anonymous';

function normalizeBrowserScope(scope: string): string {
  const value = scope.trim();
  return value && /^[A-Za-z0-9:._-]{1,160}$/.test(value) ? value : 'anonymous';
}

/** Storage is partitioned by authenticated Ripple user/workspace in Server mode. */
export function activateBrowserAuthScope(scope: string): void {
  activeBrowserScope = normalizeBrowserScope(scope);
}

/** Logout only detaches the active namespace; it never deletes another user's local history. */
export function clearBrowserAuthScope(): void {
  activeBrowserScope = 'anonymous';
}

export function browserLocalKey(suffix: string): string {
  return activeBrowserScope === 'local' ? `ripple_${suffix}` : `ripple:${activeBrowserScope}:${suffix}`;
}

export function browserSessionKey(suffix: string): string {
  return activeBrowserScope === 'local' ? `ripple_${suffix}` : `ripple:${activeBrowserScope}:${suffix}`;
}

export function browserBroadcastName(suffix: string): string {
  return `ripple-${suffix}-${activeBrowserScope.replace(/[^A-Za-z0-9._-]/g, '_')}`;
}

export function readBrowserLocalValue(suffix: string): string | null {
  try { return localStorage.getItem(browserLocalKey(suffix)); } catch { return null; }
}

export function writeBrowserLocalValue(suffix: string, value: string | null): void {
  try {
    const key = browserLocalKey(suffix);
    if (value === null) localStorage.removeItem(key); else localStorage.setItem(key, value);
  } catch { /* quota/privacy mode */ }
}

const PUBLISH_DEFAULT: PublishDraft = {
  title: '', body: '', platforms: ['xiaohongshu'], overrides: {}, tags: '',
};
export function loadPublishDraft(): PublishDraft {
  try {
    const raw = readBrowserLocalValue('publish_draft');
...
export function loadActiveId(): string | null {
  return readBrowserLocalValue('active_session');
}
export function saveActiveId(id: string | null): void {
  writeBrowserLocalValue('active_session', id);
}
...
export function loadSessions(): ChatSession[] {
  try {
    const raw = readBrowserLocalValue('sessions');
    if (!raw) return [];
