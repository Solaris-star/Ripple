...
export default function SkillDrawer({ skillName, persona, onClose, onConfigured, onNavigate }: SkillDrawerProps) {
  ...
  const settingsManaged = !!detail?.managedInSettings;
  const structured = detail?.structuredOperation || null;
  const structuredPage = structured?.module as Page | undefined;
    ...
    <div className="drawer-overlay" onClick={onClose}>
      <div className="drawer" onClick={(e) => e.stopPropagation()}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            <div>
              ...
              <div className="skill-detail-meta">
                ...
                {structured && <span className="badge badge-ok">结构化操作 · {structured.label}</span>}
              </div>
            </div>
        ...
        <div className="drawer-body">
          ...
          {settingsManaged && (
            <div className="panel">
              <div className="panel-title">模型配置由 Ripple 设置统一管理</div>
              <p style={{ fontSize: 13, lineHeight: 1.8, color: 'var(--text-secondary)' }}>
                这个 Skill 只保留工作方法与执行指导。模型服务、Provider 与 API Key 请在「设置 → 媒体生成模型」配置；用户操作和 Ripple Agent 会共用同一套配置。
              </p>
            </div>
          )}

          {structured && (
            <div className="panel r2-structured-skill-panel">
              <div className="panel-title">已接入 Ripple 模块</div>
              <p>{structured.description}</p>
              <div className="r2-structured-skill-facts">
                <span>操作 ID：{structured.id}</span><span>契约版本：v{structured.version}</span>
                <span>{structured.ready ? '当前可用' : '需要先配置 Agent 模型'}</span>
              </div>
              <button className="btn btn-primary" disabled={!structuredPage} onClick={() => { if (structuredPage) { onClose(); onNavigate(structuredPage); } }}>
                打开{structured.label}工作区
              </button>
            </div>
          )}
          ...
          {/* 执行：已迁移技能只从对应模块走结构化操作，避免双路径。 */}
          {!structured && <div className="panel">
            <div className="panel-title">▶ 运行</div>
            {blocked && (
              <div style={{ fontSize: 13, color: 'var(--amber)', marginBottom: 10 }}>
                {settingsManaged ? '该 Skill 依赖的媒体模型尚未就绪，请先到「设置 → 媒体生成模型」完成配置。' : '该 Skill 需要先配置对应的外部服务才能运行。'}
              </div>
            )}
            <textarea className="field" placeholder="输入内容，例如主题 / 素材 / 要求…" value={input} onChange={(e) => setInput(e.target.value)} style={{ minHeight: 100 }} />
            <div style={{ marginTop: 10 }}><button className="btn btn-primary" onClick={handleRun} disabled={running || !input.trim() || blocked}>{running ? <><span className="spinner" style={{ width: 14, height: 14, margin: 0 }} />执行中…</> : '执行'}</button></div>
            {runErr && <div style={{ color: 'var(--red)', fontSize: 13, marginTop: 10 }}>{runErr}</div>}
            {resultHtml && <div className="skill-result" dangerouslySetInnerHTML={{ __html: resultHtml }} />}
          </div>}
